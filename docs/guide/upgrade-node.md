...
This document describes the upgrade procedure of a `go-bitsong` full-node to a new version.
...
See the [testnet repo](https://github.com/bitsongofficial/networks) for details on which version is needed for which public testnet, and the [go-bitsong release page](https://github.com/bitsongofficial/go-bitsong/releases) for details on each release.
...
If you are joining a public testnet, fetch the genesis from the appropriate testnet in the [testnet repo](https://github.com/bitsongofficial/networks). Otherwise, fetch it from your trusted source.
...
At this point, you might want to run a script to update the exported genesis into a genesis that is compatible with your new version. For example, the attributes of a the `Account` type changed, a script should query encoded account from the account store, unmarshall them, update their type, re-marshal and re-store them. [TODO: add test script]
