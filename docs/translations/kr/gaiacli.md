...
`bitsongcli`는 사용자가 풀노드를 통해 코스모스 허브 네트워크를 사용할 수 있게하는 도구입니다. 풀노드는 사용자가 직접 운영하거나 원격 풀노드에 연결하여 사용하실 수 있습니다. `bitsongcli`를 설치하기 위해서는 [다음 과정](./installation.md)을 참고하세요.

### bitsongcli 설정하기

`bitsongcli`를 설정하는데 사용되는 주 명령어는 다음과 같습니다:
...
```bash
bitsongcli keys add <계정_명칭(account_name)>
```
...
```bash
bitsongcli keys show <계정_명칭(account_name)>
```
...
트랜잭션을 전파할때 `bitsongcli`에서 `--broadcast-mode`를 사용하실 수 있습니다. `--broadcast-mode`는 `sync`(동기, 기본 값), `async`(비동기), 또는 `block`(보류) 값으로 설정될 수 있습니다. `sync` 값은 클라이언트가 CheckTx 리턴 값을 돌려주고, `async` 값은 클라이언트가 즉시 반응을 알려주며, `block` 값은 트랜잭션이 커밋(또는 타임이웃)될 때까지 대기합니다.
...
```bash
bitsongcli tx send ... --fees=5000uatom
```
...
```bash
bitsongcli query account <코스모스_주소(account_cosmos)>
```
...
```bash
bitsongcli tx send <보내는_사람_키_명칭_또는_주소(sender_key_name_or_address)> 10faucetToken \
...
```bash
bitsongcli query account <보내는_주소(account_cosmos)>
bitsongcli query account <수신자_주소(destination_cosmos)>
```

특정 블록 높의에서의 잔고를 확인하고 싶으시다면 `--block` 플래그를 사용하실 수 있습니다:

```bash
bitsongcli query account <코스모스_주소(account_cosmos)> --block=<블록_높이(block_height)>
```
...
```bash
bitsongcli tx send <보내는이_주소(sender_address)> <수신자_주소(destination_cosmosaccaddr)> 10faucetToken \
  --chain-id=<체인_아이디(chain-id)> \
  --generate-only > unsignedSendTx.json
```

::: tip 참고
`--generate-only` 명령어는 `bitsongcli`가 로컬 키베이스를 액세스하지 않습니다. `--generate-only` 플래그를 사용하시는 경우, `<보내는_사람_키_명칭_또는_주소(sender_key_name_or_address)>` 값은 키 명칭이 아닌 주소 값을 입력하세요.
:::
...
```bash
bitsongcli tx sign --validate-signatures
  signedSendTx.json
```
...
```bash
bitsongcli query txs --events='message.sender=cosmos1...'
```

다수의 `events`를 이용하실 경우:

```bash
bitsongcli query txs --events='message.sender=cosmos1...&message.action=withdraw_delegator_reward'
```

페이지네이션은 `page`와 `limit` 값으로 지원됩니다.

```bash
bitsongcli query txs --events='message.sender=cosmos1...' --page=1 --limit=20
```
...
```bash
bitsongcli tx slashing unjail --from <검증인_오퍼레이터_주소(validator-operator-addr)>
```
...
```bash
bitsongcli query slashing signing-info <검증인_pubkey(validator-pubkey)>
```
...
```bash
bitsongcli query staking validator <cosmosval_계정(account_cosmosval)>
```
...
```bash
bitsongcli query staking delegation <위임자_코스모스_주소(delegator_addr)> <검증인_주소(validator_addr)>
...
```bash
bitsongcli query staking delegation <위임자_코스모스_주소(delegator_addr)>
```
...
```bash
bitsongcli query staking unbonding-delegation <위임자_주소(delegator-addr)> <검증인_주소(validator_addr)>
```

또는 특정 위임자의 모든 언본딩 정보를 확인하고 싶으신 경우:

```bash
bitsongcli query staking unbonding-delegations <위임자_주소(account_cosmos)>
```

추가적으로 특정 검증인으로 부터 언본딩하는 정보를 확인하고 싶으신 경우:

```bash
bitsongcli query staking unbonding-delegations-from <검증인_cosmosval_주소(account_cosmosval)>
```
...
```bash
bitsongcli query staking redelegation <위임자_주소(delegator_addr)> <기존_검증인_주소(src_val_addr)> <대상_검증인_주소(dst_val_addr)>
```

특정 위임자의 모든 검증인에 대한 재위임을 확인하고 싶으신 경우:

```bash
bitsongcli query staking redelegations <위임자_주소(account_cosmos)>
```

특정 검증인에 대한 재위임을 확인하고 싶으신 경우:

```bash
  bitsongcli query staking redelegations-from <검증인_주소(account_cosmosval)>
```
...
```bash
  bitsongcli query delegations-to <검증인_주소(account_cosmosval)>
```
...
```bash
bitsongcli tx gov submit-proposal param-change <프로포절/파일/경로/프로포절.json> \
  --from=<키_명칭(key_name)> \
  --chain-id=<체인_아이디(chain_id)>
```
...
```bash
bitsongcli query gov proposal <프로포절_ID(proposal_id)>
```
...
```bash
bitsongcli query gov proposer <프로포절_ID(proposal_id)>
```
...
```bash
bitsongcli tx gov deposit <프로포절_ID(proposal_id)> "10000000uatom" \
  --from=<키_명칭(name)> \
...
```bash
bitsongcli query gov deposits <프로포절_ID(proposal_id)>
```

특정 주소에 대한 보증금은 다음과 같이 확인하실 수 있습니다:

```bash
bitsongcli query gov deposit <프로포절_ID(proposal_id)> <보증금_제공자_주소(depositor_address)>
```
...
```bash
bitsongcli tx gov vote <프로포절_ID(proposal_id)> <Yes/No/NoWithVeto/Abstain(표 선택)> \
  --from=<키_명칭(key_name)> \
  --chain-id=<체인_아이디(chain-id)>
```
...
```bash
bitsongcli query gov vote <프로포절_ID(proposal_id)> <투표자_주소(voter_address)>
```
과거 프로포절에 대한 표 정보를 확인하기 위해서는:

```bash
bitsongcli query gov votes <프로포절_ID(proposal_id)>
```
...
```bash
bitsongcli query gov tally <프로포절_ID(proposal_id)>
```
...
```bash
bitsongcli query distribution commission <검증인_주소(validator_address)>
```
...
```bash
bitsongcli query distribution slashes <검증인_주소(validator_address)> <시작_블록_높이(start_height)> <끝_블록_높이(end_height)>
```
...
```bash
bitsongcli query distribution rewards <위임자_주소(delegator_address)> <검증인_주소(validator_address)>
```
...
```bash
bitsongcli query distribution rewards <위임자_주소(delegator_address)>
```
